<script lang="ts">
	...
	const monthOptions = Array.from({ length: 12 }, (_, i) => ({
		label: monthFmt.format(new Date(2022, i, 1, 0, 0, 0, 0)),
		value: i + 1,
	}));
	...
	{#snippet children({ months, weekdays })}
		<Calendar.Header>
			<Calendar.Heading class="flex w-full items-center justify-between gap-2">
				...
				>
					<Select.Trigger aria-label="Укажите месяц" class="w-[60%]">
						{monthLabel}
					</Select.Trigger>
				...
				>
					<Select.Trigger aria-label="Укажите год" class="w-[40%]">
						{defaultYear?.label ?? 'Выберите год'}
					</Select.Trigger>
					<Select.Content class="max-h-[200px] overflow-y-auto">
